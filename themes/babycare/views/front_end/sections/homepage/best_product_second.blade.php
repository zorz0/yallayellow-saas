<section class="two-col-slider-two-section padding-top" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
        ...
        <div class="row">
                    ...
                    <div class="product-card">
                        <div class="product-card-inner">
                            <div class="product-content-top text-center">
                                ...
                                <h4><a href="#">{{ $new_product->name }}</a>
                                </h4>
                            </div>
                            <div class="favorite-icon">
                                <a href="javascript:void(0)" class="wishbtn-globaly" product_id="{{ $new_product->id }}"
                                ...
                                <div class="product-btn-wrp">
                                    ...
                                    @if(isset($module) && $module->isEnabled())
                                        ...
                                        @include('productquickview::pages.button', ['product_slug' => $new_product->slug ?? null, 'slug' => $slug ?? null, 'currentTheme' => $currentTheme])
                                    @endif
                                    @if(isset($compare_module) && $compare_module->isEnabled())
                                        {{-- Include the module blade button --}}
                                        @include('productcompare::pages.button', ['product_slug' => $new_product->slug ?? null, 'slug' => $slug ?? null, 'currentTheme' => $currentTheme])
                                    @endif
                                </div>
                            </div>
                            <div class="product-img">
                                <a href="#">
                                    <img src="{{ asset($new_product->cover_image_path) }}"
                                        alt="product-img">
                                </a>
                            </div>
                            <div class="product-content-bottom text-center">
                                <div class="price-label2">
                                    ...
                                    <div class="price">
                                        <ins>{{ currency_format_with_sym( ($new_product->sale_price  ?? $new_product->price ), $store->id, $currentTheme) }}</ins>
                                    </div>
                                    @else
                                    <div class="price">
                                    ...
                                    {!! \App\Models\Product::productSalesPage($currentTheme, $slug, $new_product->id) !!}
                                </div>
            ...
            <div class="col-md-12 col-sm-12 col-lg-6 col-12">
                <div class="two-colum-img-card">
                    <img src="{{ asset(
                    $section->best_product_second->section->image->image ?? '') }}" class="products-card {{ $section->best_product_second->section->image->slug ?? '' }}_preview" alt="image">
                    <div class="label">{{ __('CATEGORIES') }}</div>

                    <div class="two-colum-img-content">
                        <div class="card-title">
                        <h3 id="{{ $section->best_product_second->section->sub_title->slug ?? '' }}_preview">{!!
                $section->best_product_second->section->sub_title->text ?? '' !!}</h3>
                                        <p id="{{ $section->best_product_second->section->description->slug ?? '' }}_preview">{!!
                $section->best_product_second->section->description->text ?? '' !!}</p>
                            <a href="#" class="btn theme-btn" id="{{ $section->best_product_second->section->button->slug ?? '' }}_preview">
                                            {!!
