<section class="best-seller-section bestseller-second padding-top padding-bottom" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
                ...
                <div class="product-card ">
                    <div class="product-card-inner">
                        <div class="product-content-top text-center">
                            <div class="label">{{ $items->label->name ?? '' }}
                            </div>
                            <h4><a href="{{ url($slug.'/product/'.$items->slug) }}">{{ $items->name }}</a> </h4>
                                <div class="favorite-icon">
                                    ...
                                    <div class="product-btn-wrp">
                                        ...
                                        @if(isset($module) && $module->isEnabled())
                                            ...
                                            @include('productquickview::pages.button', ['product_slug' => $items->slug ?? null, 'slug' => $slug ?? null, 'currentTheme' => $currentTheme])
                                        @endif
                                        @if(isset($compare_module) && $compare_module->isEnabled())
                                            {{-- Include the module blade button --}}
                                            @include('productcompare::pages.button', ['product_slug' => $items->slug ?? null, 'slug' => $slug ?? null, 'currentTheme' => $currentTheme])
                                        @endif
                                    </div>
                                </div>
                        </div>
                        <div class="product-img">
                            <a href="{{ url($slug.'/product/'.$items->slug) }}">
                                <img src="{{ asset($items->cover_image_path) }}" alt="product-img">
                            </a>
                        </div>
                        <div class="product-content-bottom text-center">
                            <div class="price-label2">
                                @if ($items->variant_product == 0)
                                    <div class="price">
                                        <ins>{{ currency_format_with_sym( ($items->sale_price  ?? $items->price ), $store->id, $currentTheme) }} </ins>
                                    </div>
                                @else
                                    <div class="price">
                                ...
                                {!! \App\Models\Product::productSalesPage($currentTheme, $slug, $items->id) !!}
