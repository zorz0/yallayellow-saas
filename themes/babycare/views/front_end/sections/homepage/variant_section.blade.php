...
style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <img src="{{ asset($section->variant_background->section->image->image ?? 'themes/'.$currentTheme.'/assets/img/banner-sec-7.png')}}" id="{{ $section->variant_background->section->image->slug ?? '' }}_preview" alt="">
    <div class="custome_tool_bar"></div>
            <div class="container">
                <div class="common-title">
                    ...
                    <a href="#" class="btn-secondary white-btn" id="{{ $section->variant_background->section->button->slug ?? '' }}_preview">
