            ...
            <div class="mini-cart-item">
                ...
                <div class="mini-cart-details">
                    ...
                    <div class="qty-spinner">
                        ...
                        <input type="text" class="quantity 45_quatity" data-cke-saved-name="quantity" name="quantity" value="{{ $item->qty }}" min="01" id="cart_list_sidebar_quantity{{ $item->qty }}" data-id="463">
                    ...
                    <div class="pvarprice d-flex align-items-center justify-content-between">
                        <div class="price">
                            <ins>{{ currency_format_with_sym( $item->final_price, $store->id, $currentTheme) ?? SetNumberFormat($item->final_price) }} </ins><del>{{ currency_format_with_sym( $item->total_orignal_price, $store->id, $currentTheme) ?? SetNumberFormat($item->total_orignal_price) }}</del>
                        </div>
                        <a class="remove_item remove_item_from_cart" title="Remove item" href="#" data-id="{{ $item->cart_id }}">
    ...
    <div class="mini-cart-footer mini-cart-footer1">
        @if (!empty($response->data->cart_total_product))
            <div class="u-save d-flex justify-end">
                {{ __("Subtotal") }}: {{  currency_format_with_sym( ($response->data->final_price ?? ($response->data->sub_total ?? 0)), $store->id, $currentTheme) ?? SetNumberFormat($response->data->final_price ?? ($response->data->sub_total ?? 0)) }}
            </div>
            <div class="u-save d-flex justify-end">
                {{ __("Tax") }}: {{ currency_format_with_sym($response->data->tax_price, $store->id, $currentTheme) ?? SetNumberFormat($response->data->tax_price) }}
            </div>
            @php
